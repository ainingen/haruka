...
| `slots.json` | スロットごとの解説文（`slot_description`）。UI の表示専用で、計算には使わない |
| `sponsors.json` | スポンサーの会社（架空名）。段階1〜4に3社ずつ。段階が上がったときに1社がランダムに付く |
| `economy.json` | お金と進行の数値：初期資金、賞金表、参加費、スポンサー料、ポイント、昇降格の線、消耗と修理。**`tools/generate-economy.mjs` が書き出す**（賞金表は式で作る）。設計は `docs/設計/経済とシーズン.md` |
...
| `fuel_consumption` | 燃料消費率 | **不利** | 1周で使う燃料が増える。積む量が増えるぶん車が重くなり、足りなければ燃料切れでリタイア |
...
| `short` | **人が呼ぶときの短い名**（ながさわ／みさき／ひばり平／かざはや／あさひな）。`name` の頭と一致する。ハルカのノート・親父のノート・無線はこちらで呼ぶ。実況と解説、コース選択は `name`（場内アナウンスと一覧なので正式名でいい） |
...
スロットそのものの解説。**表示専用で、レース計算はこのファイルを読まない。**
`src/ui/setup.html` のパーツ選択モニターが、候補リストの下に出す。

トップレベルは **スロット id → オブジェクト**。スロット id は上の「スロット一覧」と同じ語彙で、
`src/engine/race.js` の `SLOTS` に無いキーを書くとテストが落ちる。

```json
{
  "intake": {
    "slot_description": "エンジンが空気を吸い込む通り道。抵抗を減らすほど……"
  }
}
```
...
| フィールド | 型 | 意味 |
|---|---|---|
| `slot_description` | string | そのスロットが何で、何と引き換えに何が得られるかの説明。2〜3文 |

**全スロットを埋める必要はない。** 書いていないスロットでは、モニターの解説枠そのものが出ない。
書けたところから足していく（現在は `engine` の5スロットのみ）。

### 書き方

パーツの `note` とは**話者も層も違う**。取り違えないこと。

| | `parts.json` の `note` | `slots.json` の `slot_description` |
|---|---|---|
| 話者 | 父・大河 | 誰でもない（説明文） |
| 層 | 紙（`docs/設計/画面設計.md`） | 画面（モニター） |
| 中身 | そのパーツを買うかどうかの判断材料。答えは書かない | そのスロットの仕組みと、避けられないトレードオフ |
| 文体 | 短文・命令形（「〜を数えろ」） | 常体の説明。命令形にしない |

1. **仕組み → 得るもの → 失うもの**の順で書く。失うものを省かない（副作用の無いパーツを作らないのと同じ理由）
2. **個別のパーツ名と数値を書かない。** パーツが増減しても直さなくて済むようにする
3. **解禁クラスを書かない。** 候補リストが「クラス4から」と既に出している
4. 実在のブランド名・メーカー名は使わない。一般的な技術用語（過給、キャンバー、LSD）は問題ない
...
父・大河が書き残した、コース × クラスごとの基準セッティング。`src/ui/setup.html` の「ノート通りにする」が丸ごと写す。

| フィールド | 意味 |
|---|---|
| `course` / `class` | どのコースの、どのクラスのページか。クラス1〜3は走れる全コースにあり、クラス4は一部、クラス5は無い（父の記述が尽きる） |
| `parts` | 装着するパーツ id。すべてそのクラスの規定内、スポンサー段階は `class − 1` 以下 |
| `settings` | 連続値（空気圧、車高、ブレーキ配分）。解禁されていないものは書かない |
| `note` | 大河の書き込み。短文・命令形。答えは書かず、何を見るかを書く |
| `quali` | 予選用。`note`（予選の書き込み）と `settings`（決勝用に上書きする値。空気圧は必ず高め） |

## ai-baseline.json

**AI 専用のクラス5の基準構成。親父のノートではない。** 形は `notes.json` と同じだが、
`note`（書き込み）を持たない。大河はクラス4までしか走っていないので、そこに書く文章が無い。

- クラス5でも AI は走るので、内部にだけ基準を置く。クラス4のノートを、
  クラス5で解禁されるワークス部品に置き換えたもの
- **プレイヤーの画面には出さない。** `src/ui/setup.html` はこのファイルを読まないので、
  「ノート通りにする」はクラス5では出ない（`データは setup.html が読む notes.json だけ`）。
  読むのは `src/ui/race.html` と `tools/simulate-season.mjs`。合成は `rivals.js` の `aiNotes()`
- ここを setup.html から読ませてはいけない。テスト S7 が見張っている
...
| `chat` | 雑談系。`general` / `with_reply`（一言返せるもの）/ `rare`（稀な一言）/ `quali`（予選ネタ）/ `ai`（相手車の性格別）/ `course_start`（コース ID → 開始直後の一言） |
| `quali` | 予選専用。`out` / `attack_start` / `attack_good` / `attack_bad` / `traffic` / `in` / `done` |
| `replies` | 情報系への返事3択。`consistency` が返事による調子の増減（レース中にセッティングは変えられない） |
| `pit` | セッティング画面での一言。`pitComment()` が返す ID → `{ mood, lines }`。予選タブでは `quali_pressure_ok`、ノートを写した直後は `from_notes` |
...
| リタイア判定 | `reliability` |
| 燃料 | `fuel_consumption`。走る周回数ぶん＋余裕を積んで出走し、周ごとに減る。積んでいる量がそのまま `weight` に乗るので、満タンは遅く、減るほど速くなる。尽きたらリタイア（`WEIGHTS.fuel`） |
| **未使用** | `noise`（音量規定による出走可否）。パーツの `durability` もレース間の消耗として別途扱う予定 |
